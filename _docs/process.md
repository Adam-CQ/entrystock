...
- `_docs/architecture.md` is the source of truth for the technical architecture,
  selected stack, system boundaries, and data responsibilities.
- `_docs/plan.md` is the source of truth for the product scope, PoC goals, and
  planned workflow.
- Read both documents before grooming or implementing an issue. Do not import
  assumptions from documents belonging to another project.
- If an issue conflicts with either document, resolve the conflict explicitly
  by updating the issue or the relevant project document before implementation.
...
- `.env` copied from the main checkout, with `DATABASE_URL` pointed at a
  database of its own, `entrystock_wt<issue>`
- `CREATE DATABASE entrystock_wt<issue>` inside the Postgres container. If
...
- Commands are run with the database named explicitly:
  `DATABASE_URL=postgres://postgres:postgres@localhost:5432/entrystock_<issue> uv run pytest`
...
- Write temporary files to the session scratchpad, which is outside the
  ...
  code
- Leave worktrees, branches and `entrystock_wt<issue>` databases in place
